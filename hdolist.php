...
<td class=menutopbar height=22 style="white-space: nowrap"><font color="#222222"><b>HDO List</b></font></td>
...
<tr><td colspan=5 width=100%><div class=tab>Help Desk Operators</td></tr>
<tr><td width=15%><div class=tab>Username:</td><td width=25%><div class=tab>Promoted By:</td><td width=15%><div class=tab>Pen Points:</td></tr><?
$entertainers = mysql_query("SELECT * FROM hdos");

while($etainers = mysql_fetch_array($entertainers)){
$name = $etainers['username'];
$by = $etainers['promotedby'];
$nameinfo = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE username = '$name'"));
$status = $nameinfo['status'];
$pts = $nameinfo['penpoints'];
if($status == 'Dead'){$color = 'crimson';$fcolor = '<font color=black face=verdana size=1><b>';}else{$color = '#222222';$fcolor = '<font color=khaki face=verdana size=1><b>';}
