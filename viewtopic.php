...
<table cellspacing="0" class="resize" id="block">
    <tr>
        ...
        <td valign="top" class="main">
            ...
            $editsql = mysql_query("SELECT * FROM forumtopics WHERE topicid = '$topicid' AND esc = '1'");
            $editarray = mysql_fetch_array($editsql);
            $editname = $editarray['creator'];
            $posttits = mysql_num_rows(mysql_query("SELECT * FROM forumtopics WHERE topicid = '$topicid' AND esc = '1'"));
            $noposts = mysql_num_rows(mysql_query("SELECT * FROM forumposts WHERE topicid = '$topicid' AND esc = '1'"));
            ...
            if(($rankid >= '25')||($editname == $gangsterusername)||($ahdoperson > '0')){
                if(isset($_POST['edit'])) { if(!$edittopic){}else{
                    mysql_query("UPDATE forumtopics SET info = '$edittopic' WHERE topicid = '$topicid' AND esc = '1'");
                }}}
            ...
            $dreplace[15] = "<a href=viewprofile.php?username=\"$1\"><b>$1</b></a>";
            ...
            if($_GET['follow'] == 1){
                $checkif = mysql_num_rows(mysql_query("SELECT * FROM followtopic WHERE username = '$usernameone' AND topicid = '$topicid'"));
                if($checkif > 0){  }
                else{
                    mysql_query("INSERT INTO `followtopic` (username,topicid) VALUES ('$usernameone','$topicid')");
                    $showoutcome++; $outcome = "You are now following this topic!";
                }}
            ...
            if($_GET['follow'] == 2){
                ...
                if($checkif < 1){  }
                    ...
                    mysql_query("DELETE FROM followtopic WHERE username = '$usernameone' AND topicid = '$topicid'");
                    $showoutcome++; $outcome = "You are no longer following this topic!";
                }}
            ...
            if(isset($_POST['dopost'])) {
                ...
                if($mutedusernamerows > '0'){die("<font color=white face=verdana size=1>Username: <b>$mutedusernameone</b> and IP: <b>$mutedipone</b> have been muted! Contact a member of <b>Staff</b> to be unmuted!");}
                elseif($mutediprows > '0'){die("<font color=white face=verdana size=1>Username: <b>$mutedusernametwo</b> and IP: <b>$mutediptwo</b> have been muted! Contact a member of <b>Staff</b> to be unmuted!");}

                if(!$newpost){}
                elseif($toplocked == 'yes'){$showoutcome++; $outcome = "<font color=C11B17 face=verdana size=1><b>This topic has been locked!</b></font>";}
                elseif($topiccheck == '0'){$showoutcome++; $outcome = '<font color=red face=verdana size=1><b>No such topic!</b></font><META HTTP-EQUIV="Refresh" CONTENT="2; URL=forum.php">';}
                else{
                    if(($senior == '1')&&($playerrank < '25')){$playerrank = '123';}
                    $gettagraw = preg_replace_callback('/@([a-zA-Z0-9_]+)/',create_function('$matches','return \'.$matches[1].\'</a>\';'),$newpost);
                    $allowed = "/[^a-z0-9]/i";
                    $gettag = preg_replace($allowed,"",$gettagraw);
                    mysql_query("UPDATE forumtopics SET time = '$time' WHERE topicid = '$topicid' AND esc = '1'");

                    $post_time = date('Y-m-d H:i:s');
                    
                    if(isset($_POST['reply_user'])){
                        $reply_user=$_POST['reply_user'];
                        mysql_query("INSERT INTO forumposts(username,topicid,ip,time,type,post,rankid,esc,reply)
VALUES ('$gangsterusername','$topicid','$userip','$post_time','main','$newpost','$playerrank','1','$reply_user')");
                    }else{
                        mysql_query("INSERT INTO forumposts(username,topicid,ip,time,type,post,rankid,esc)
VALUES ('$gangsterusername','$topicid','$userip','$post_time','main','$newpost','$playerrank','1')");
                    }
                    mysql_query("UPDATE users SET posts = (posts + 1) WHERE username = '$usernameone'");
                    $sql = "SELECT username FROM users WHERE username='$gettag'";

                    $query = mysql_query($sql) or die(mysql_error());
                    $gettagnum = mysql_num_rows($query);

                    $getpost = preg_replace_callback(
                        '/@([a-zA-Z0-9_]+)/',
                        create_function('$matches','return \'<b><font color=yellow>@</font></b><a href="viewprofile.php?username=\'.$matches[1].\'" style="color:yellow;"><b>\'.$matches[1].\'</b></a>\';'),$string);
                    preg_match_all("/@([a-zA-Z0-9_]+)/", $newpost, $matches);
                    $i=1;
                    foreach($matches[1] as $match){
                        mysql_query("UPDATE users SET notify = '1',notification = '$usernameone has tagged you in a post click <a href=viewtopic.php?topicid=$topicid><font color=yellow><b>here</font></b></a> to view topic!' WHERE username = '$match'") or die(mysql_error());
                    }
                    $i++;
                    ...
                    $getfollowerss = mysql_query("SELECT * FROM followtopic WHERE username != '$usernameone' AND topicid = '$topicid' ORDER BY id");
                    while($getfollowers = mysql_fetch_array($getfollowerss)){
                        $followuser = $getfollowers['username'];
                        mysql_query("UPDATE users SET notify = '1',notification = 'A post has been made in a topic you are <a href=viewtopic.php?topicid=$topicid><font color=royalblue><b>following</font></b>' WHERE username = '$followuser'");
                    }}}

            if($usernameone == $topiccreator){
                mysql_query("UPDATE forumtopics SET new = '0' WHERE topicid = '$topicid'");}

            if(($rankid >= '25')||($ahdoperson > '0')){
                if(isset($_POST['delete'])) {
                    mysql_query("DELETE FROM forumtopics WHERE esc = '1' AND topicid = '$topicid'");
                    mysql_query("DELETE FROM forumposts WHERE esc = '1' AND topicid = '$topicid'");
                }}

            if(($rankid >= '25')){
                if(isset($_POST['dj'])) { mysql_query("UPDATE forumtopics SET id = '4' WHERE topicid = '$topicid' AND esc = '1'");
                }}

            if(($rankid >= '25')){
                if(isset($_POST['sticky'])) {
                    mysql_query("UPDATE forumtopics SET id = '2' WHERE topicid = '$topicid' AND esc = '1'");
                }}

            if(($rankid >= '25')){
                if(isset($_POST['important'])) {
                    mysql_query("UPDATE forumtopics SET id = '3' WHERE topicid = '$topicid' AND esc = '1'");
                }}

            if($rankid == '100'){
                if(isset($_POST['update'])) {
                    mysql_query("UPDATE forumtopics SET id = '4' WHERE topicid = '$topicid' AND esc = '1'");
                }}

            if(($rankid >= '25')){
                if(isset($_POST['undo'])) {
                    mysql_query("UPDATE forumtopics SET id = '1' WHERE topicid = '$topicid' AND esc = '1'");
                }}

            if(($topiccreator == $gangsterusername)||($rankid >= '25')||($hdoperson >= '1')){
                if(isset($_GET['lock'])) { $showoutcome++; $outcome = '<font color=white face=verdana size=1>Topic locked</font>';mysql_query("UPDATE forumtopics SET locked = 'yes' WHERE topicid = '$topicid' AND esc = '1'");}
            }

            if(($topiccreator == $gangsterusername)||($rankid >= '25')||($hdoperson >= '1')){
                if(isset($_GET['unlock'])) {$showoutcome++; $outcome = '<font color=white face=verdana size=1>Topic Unlocked</font>';mysql_query("UPDATE forumtopics SET locked = 'no' WHERE topicid = '$topicid' AND esc = '1'");}
            }
            ...
            if(($rankid >= '25')||($hdoperson > '0')){
                if(isset($_GET['deletepost'])) {
                    mysql_query("DELETE FROM forumposts WHERE esc = '1' AND topicid = '$topicid' AND id = '$deletepost'");
                    mysql_query("UPDATE forumtopics SET time = '$time' WHERE topicid = '$topicid' AND esc = '1'");
                }}

            $gettopicid = mysql_query("SELECT * FROM forumtopics WHERE topicid = '$topicid' AND esc = '1'");
            $gettopicidarray = mysql_fetch_array($gettopicid);
            $exists = mysql_num_rows($gettopicid);
            if($exists == '0'){$showoutcome++; $outcome = '<font color=red face=verdana size=1><b>No such topic!</b></font><META HTTP-EQUIV="Refresh" CONTENT="2; URL=forum.php">';}
                            ...
                            <div style="display: table; width: 100%; padding: 9px; padding-top: 5px; padding-bottom: 5px;">
                                ...
                                <div style="display: table-cell; text-align: right;">
                                    ...
                                    if(($topiccreator == $gangsterusername)||($rankid >= '25')||($hdoperson > '0')){
                                        ...
                                        echo"<a href=viewtopic.php?topicid=$topicid&lock=1 style=color:$cu;><b> Lock </b></a>";}
                                    if(($rankid >= '25')||($ahdo > '0')){
                                        if($toplocked == 'yes'){$cu = 'white';}
                                        else{$cu='white';}
                                        echo"<span class=\"miniSep\"> - </span><a href=viewtopic.php?topicid=$topicid&unlock=1 style=color:$cu;><b>Unlock </b></a>";
                                    }?>
                                </div>
            ...
            <div id=editee style="display:none;" name=edit>
                ...
                <form action="viewtopic.php?topicid=<?echo$topicid;?>" method="post" name="smileys" style="text-align: center;">
            ...
            ?>
                        <form action = "viewtopic.php?topicid=<?echo$topicid;?>" method = "post" style="padding: 7px; text-align: center;">
            ...
            <script language="javascript">
                ...
                <?php
                $getposts = mysql_query("SELECT * FROM forumposts WHERE topicid = '$topicid' AND esc = '1' ORDER BY id DESC LIMIT $selectfrom,$selectto ");
                ...
                $ereplace[17] = "<a href=logout.php?id=$sessionid>Click me</a>";
                ...
                $trololol = str_replace($fpattern, $freplace, $postinforawb, $countthree);
                $postinfo = preg_replace_callback('/@([a-zA-Z0-9_]+)/',create_function('$matches','return \'<b><font color=yellow>@</font></b><a href="viewprofile.php?username=\'.$matches[1].\'" style="color:yellow;"><b>\'.$matches[1].\'</b></a>\';'),$trololol);
            ...
            <table class="menuTable curve10px" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    ...
                    <input type ="submit" value="NORMAL" class=\'textarea inline_form curve3px\' name="undo">';
                            ...
                            <div style="margin: auto; max-width: 413px; margin-top:10px;">
                            <form action="viewtopic.php?topicid=<?echo$topicid;?>" method="post" name="smiley" style="text-align:center;">
