...
function updateCasinoInfo(i, action, location, name) {
    $('#casinoLoader' + i).fadeIn(150, function () {
        if (action == 1) {
            var parameter= new Object;
            parameter['location']=location;
            parameter[name]=1;
            $.post("/manage_propierties.php?cpanel=true", parameter).done(function () {
            ...
            var parameter= new Object;
            parameter['location']=location;
            parameter[name]=document.getElementById('newMax' + i).value;
            $.post("/manage_propierties.php?cpanel=true", parameter).done(function (data) {
                console.log(data);
                if (data == "-1") {
            ...
            var parameter= new Object;
            parameter['location']=location;
            parameter[name]=document.getElementById('newOwner' + i).value;
            $.post("/manage_propierties.php?cpanel=true", parameter).done(function (data) {
            ...
            var parameter= new Object;
            parameter['location']=location;
            parameter[name]=document.getElementById('newPrice' + i).value;
            $.post("/manage_propierties.php?cpanel=true", parameter).done(function (data) {
...
function checkToggle() {
    ...
    var all_true = false;
    $("." + toToggleClass).each(function() {
    	if ($(this).prop("checked") == false) {
			$("." + toToggleClass).prop("checked", true);
			all_true = true;
		}
    });
    if (all_true == true) return false;
    else {
		var i = 0;
	    $("." + toToggleClass).each(function() {
