    ...
    <tr>
        ...
        <td valign="top" class="main">
            ...
            if(isset($_POST['dodemote'])){
                ...
                $status = $array['status'];
                $equery = mysql_query("SELECT * FROM hdos WHERE username = '$name'");
                $erows = mysql_num_rows($equery);
                if($rows < '1'){$showoutcome++; $outcome = "<font color=white face=verdana size=1>No such user!</font>";}
                elseif($erows == '0'){$showoutcome++; $outcome = "<font color=white face=verdana size=1>User is not an HDO!</font>";}
                else{
                    $showoutcome++; $outcome = "<font color=white face=verdana size=1>User <b>$name</b> is no longer a HDO!</font>";
                    mysql_query("DELETE FROM hdos WHERE username = '$name'");
                    mysql_query("UPDATE users SET hdo= '0' WHERE username = '$name'");
                    mysql_query("DELETE FROM home WHERE username = '$name' AND type = '3'");}
            } else{
                if(isset($_POST['demote'])) {
                    $query = mysql_query("SELECT * FROM users WHERE username = '$name'");
                    $rows = mysql_num_rows($query);
                    $equery = mysql_query("SELECT * FROM users WHERE username = '$name' AND hdo ='1'");
                    $erows = mysql_num_rows($equery);
                    $array = mysql_fetch_array($query);
                    $status = $array['status'];
                    if($rows < '1'){$showoutcome++; $outcome = "<font color=white face=verdana size=1>No such user!</font>";}
                    elseif($status != 'Alive'){$showoutcome++; $outcome = "<font color=white face=verdana size=1>That user is dead!</font>";}
                    elseif($erows > '1'){$showoutcome++; $outcome = "<font color=white face=verdana size=1>User is already an HDO!</font>";}
                    else{
                        mysql_query("INSERT INTO hdos(username,promotedby,type) VALUES ('$name','$gangsterusername','normal')");
                        mysql_query("DELETE FROM jail WHERE username = '$name'");

                        mysql_query("UPDATE users SET hdo = '1', reward = '0' WHERE username = '$name'");
                        $homeinfo = "was promoted to <font color=lime>\[b\]Help Desk Operators\[\/b\]</font>!";
                        mysql_query("INSERT INTO home(`username`,`update`,`type`) VALUES ('$name','$homeinfo','3')");
                        $showoutcome++; $outcome = "<font color=white face=verdana size=1>User <b>$name</b> is now an HDO! <b>YOU</b> now take responsibility for their actions!</font>";
                    }}}
            ?>
